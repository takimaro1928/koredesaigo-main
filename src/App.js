    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        header.forEach((col, index) => {
          // values[index] が undefined の場合も考慮し、trim() 前にチェック
          const value = values[index];
          questionData[col] = typeof value === 'string' ? value.trim().replace(/^"|"$/g, '').replace(/""/g, '"') : '';
        });
      ...
      importedQuestions.forEach(iq => {
        ...
        const newQuestionData = {
          id: questionId,
          number: parseInt(iq.QuestionNumber, 10) || 0,
      ...
      console.log(`CSV Data imported: ${importedQuestions.length} questions processed.`);
      alert(`CSVから ${importedQuestions.length} 件の問題データを処理しました。`);
      return true;
